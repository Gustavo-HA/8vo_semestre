...
!    and functions.
!
!    Local, integer ( kind = 4 ) MAXNEW, the maximum number of Newton steps 
!    to take in one iteration.
!
...
!
!    Local, integer ( kind = 4 ) MAXSEC, the maximum number of secant steps 
!    to take.
!
...
!
!    Local, integer ( kind = 4 ) NBLEFT, the column at which the left corner 
!    of the bump lies.
!
!    Local, integer ( kind = 4 ) NBRITE, the column at which the right corner 
!    of the bump lies.
!
...
!
!    Local, integer ( kind = 4 ) NODE(MAXELM,NNODES), contains, for each 
!    element, the global node indices of the element nodes.
!
...
!
!    Local, integer ( kind = 4 ) NX, controls the spacing of nodes and elements 
!    in the X direction.  There are 2*NX+1 nodes in the X
!    direction.
!
!    Local, integer ( kind = 4 ) NY, controls the spacing of nodes and elements 
!    in the Y direction.  There are 2*NY+1 nodes in the Y
!    direction.
!
...
!
!    Local, integer ( kind = 4 ) XY_UNIT, the unit to which XY graphics 
!    is written.
!
...
!
  ...
  xbrite = 3.0D+00
  xlngth = 10.0D+00
  xprof = 4.0D+00 !cambio
  xy_file = 'xy_000.txt'
  ylngth = 3.0D+00 !change
...
!
  ...
  call uv_write ( f, indx, uv_unit, neqn, np, yc,xc)
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) IT, the element in which the basis function 
!    is defined.
!
...
!
!    Input, integer ( kind = 4 ) ML, MU, the number of diagonals below and 
...
!
!    Input, integer ( kind = 4 ) NEQN, the number of finite element 
!    coefficients.
!
...
!    is 6 for the quadratic triangular elements in use here.
!
!    Input, integer ( kind = 4 ) NODE(NELEMN,NNODES), contains, for each
!    element, the global node indices of the element nodes.
!
...
!
!    Output, integer ( kind = 4 ) IGETL, the index of the node in the profile 
!    line at which global unknown I occurs, or -1 if there is no such entry.
!
  implicit none
...
!
!    Input, integer ( kind = 4 ) IQ, the index of a basis function 
...
!    the reference triangle.
!
!    Input, integer ( kind = 4 ) IN, the index of a basis function 
!    in the reference triangle.
!
...
!
!    Input, integer ( kind = 4 ) IT, the index of the element 
!    containing (XQ,YQ).
!
!    Input, integer ( kind = 4 ) NELEMN, the number of elements.
!
...
subroutine uv_write ( f, indx, uv_unit, neqn, np, yc, xc )
...
!    index of the U and V velocities at that node, or 0.
!
!    Input, integer ( kind = 4 ) UV_UNIT, the FORTRAN unit number associated
...
!
  ...
  do ip = 1, np
    ...
    write ( uv_unit, '(2x,g14.6,2x,g14.6,2x,g14.6,2x,g14.6,2x,g14.6)' ) xc(ip),yc(ip),u, v,sqrt(u**2+v**2)
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) XY_UNIT, the FORTRAN unit number associated 
!    with the output file.
!
!    Input, integer ( kind = 4 ) NP, the number of nodes.
!
